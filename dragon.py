...
"""
The game where you become the Dragon keeper. Care of it's needs carefully.
Game is over if dragon's health drops to zero.
"""
...
class Dragon:
    ...
    def exhausted (self):
        return self.energy < 14 

    @property # attribute hunger    
    def hungry (self):
        return self.fullness < 14

    def feeding (self): 
        """
        We can feed the dragon. Besides fullness another values can be changed dependind on what kind of food it eats.
        """
        ...
        if answer == 1:
            ...
            print(f"{self.name} filled up on dog keeper and his Great Dane in the park. It was exhausting. {self.name} is sad and dirty from this action. Visitors of the park seen it so it's reputation decreased.")
        elif answer == 2:
            ...
            print(f"{self.name} sneaked to neighbour's courtyard and ate a hen. The fight was noisy and brought neighbour. The dragon was little sad from it. It makes it dirtier.")
        elif answer == 3:
    ...
    def play (self):
        """
        If it isn't hungry or exhausted, it cannot play. It brings him happiness. Beware mischievous dragons!  
        """
        if self.hungry:
            ...
            if self.exhausted:
                ...
                if answer == 1:
                    ...
                    self.happiness = self.happiness + 5
                    print(f"The woods gave {self.name} energy so it is little weary from the running but happy.")
                elif answer == 3:
                    self.energy = self.energy - 1
                    self.happiness = self.happiness + 5
                    self.education = self.education + 2
                    print(f"{self.name} is happy. Putting lego together did well. It trained it's brain.")
                else:
                    print("Answer must be a number.")
        dragon.vrite_out()
    ...
    def learn (self):
        """
        If it is exhausted, dragon cannot train it's brain.
        """
        if self.exhausted:
                print(f"{self.name} is exhausted.")
        else:
            ...
            if answer == 1:
                ...
                self.happiness = self.happiness + 1
                print(f"{self.name} is happier and more educated.")
            elif uceni == 2:
                self.energy = self.energy - 3
                self.education = self.education + 3
                self.happiness = self.happiness + 2
                print(f"{self.name} is exhausted and happy because it solved hard calculation.")
            else:
                print("Answer must be a number.")
        dragon.vrite_out()

    def work (self):
        # If it is hungry or exhausted, it cannot work.
        if self.hungry:
            print(f"{self.name} is hungry.")
        else:
            if self.exhausted:
                ...
                if answer == 1:
                    ...
                    self.happiness = self.happiness + 2
                    self.fullness = self.fullness + 3
                    print(f"{self.name} is happy about itself. It is exhausted and got dirty, but old lady gave it big chunk of boar to eat.")
                else:
                    print("Answer must be a number.")
            dragon.vrite_out()
    ...
    def action (self):
        """
        The whole game's control. It asks gamer what he wants to do until his dragon have at least little health. 
        """
...
"""
Gamer can choose from new or saved game. New game has starting statistic, saved game brings saved statistics from module 'value.py' and it takes data from 'data.txt'   
